...
<section class="receipt rounded-lg border border-border bg-white p-5 shadow-sm">
    <div class="text-center">
        @if($settings->business_logo)
            <img src="{{ asset('storage/'.$settings->business_logo) }}" class="mx-auto mb-2 h-14 w-14 object-contain" alt="Logo">
        @elseif(file_exists(public_path('logo.png')))
            <img src="{{ asset('logo.png') }}" class="mx-auto mb-2 h-14 w-14 object-contain" alt="Logo">
        @endif
    ...
    <div class="mt-4 border-t border-dashed border-border pt-3">
        ...
        @forelse($order->payments as $payment)
            <div class="flex justify-between text-xs">
                <span>{{ str_replace('_', ' ', ucfirst($payment->payment_type)) }} - {{ $payment->paid_at?->format('M d, h:i A') }}</span>
