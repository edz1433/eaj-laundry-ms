        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm">
                ...
                <tbody class="divide-y divide-border dark:divide-gray-800">
                    @forelse($transactions as $transaction)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-right">
                                ...
                                <p class="text-xs text-muted">Bal. {{ $appSettings?->currency ?? 'PHP' }} {{ number_format((float) $transaction->balance, 2) }}</p>
                            </td>
                            <td class="px-4 py-3">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-4 py-10 text-center text-muted">No PO transactions found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div x-cloak x-show="editOpen === {{ $transaction->id }}" x-transition class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div @click.outside="editOpen = null" class="w-full max-w-md rounded-lg bg-white p-5 shadow-2xl dark:bg-gray-900">
                ...
                <form method="POST" action="{{ route('admin.po-transactions.update', $transaction) }}" class="space-y-4">
                    ...
                    <div>
                        <label class="mb-1.5 block text-sm font-medium">Paid Amount</label>
                        <input type="number" step="0.01" min="0" max="{{ $transaction->amount }}" name="paid_amount" value="{{ $transaction->paid_amount }}" class="h-9 w-full rounded-md border border-border bg-white px-3 text-sm dark:border-gray-700 dark:bg-gray-950">
                    </div>
