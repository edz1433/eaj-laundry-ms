...
>
    ...
    <div x-show="tab === 'sales'" class="grid gap-4 xl:grid-cols-2">
        <x-report-table title="Sales by Date">
            <x-slot:head><th class="px-4 py-3">Date</th><th class="px-4 py-3 text-right">Payments</th><th class="px-4 py-3 text-right">Cash</th><th class="px-4 py-3 text-right">GCash</th><th class="px-4 py-3 text-right">Bank</th><th class="px-4 py-3 text-right">Sales</th></x-slot:head>
            ...
            @empty
                <tr><td colspan="6" class="px-4 py-10 text-center text-muted">No sales found.</td></tr>
            @endforelse
        </x-report-table>

        <x-report-table title="Sales by Branch">
            <x-slot:head><th class="px-4 py-3">Branch</th><th class="px-4 py-3 text-right">Payments</th><th class="px-4 py-3 text-right">Cash</th><th class="px-4 py-3 text-right">GCash</th><th class="px-4 py-3 text-right">Bank</th><th class="px-4 py-3 text-right">Sales</th></x-slot:head>
    ...
    <div x-show="tab === 'expenses'" class="space-y-4">
        <div class="grid gap-3 md:grid-cols-3">
            ...
            <div class="rounded-lg border border-border bg-white p-4 shadow-sm dark:border-gray-800 dark:bg-gray-900 md:col-span-3">
                <p class="text-xs text-muted">Cash Advance</p>
                <p class="mt-1 text-lg font-semibold">{{ $settings->currency ?? 'PHP' }} {{ number_format((float) $cashAdvanceTotal, 2) }}</p>
            </div>
        </div>
        ...
        <x-report-table title="Expenses">
            ...
            @forelse($expenses as $expense)
                <tr><td class="px-4 py-3">{{ $expense->expense_date?->format('M d, Y') }}</td><td class="px-4 py-3">{{ $expense->branch?->name }}</td><td class="px-4 py-3"><p class="font-medium">{{ $expense->title }}</p><p class="text-xs text-muted">{{ $expense->category }} - {{ \App\Support\StatusBadge::label($expense->expense_type ?? 'regular') }}{{ $expense->payment_method ? ' - '.$expense->payment_method : '' }}</p></td><td class="px-4 py-3">{{ $expense->paid_from === 'owner' ? 'Owner Paid / Record Only' : 'Store Cash' }}</td><td class="px-4 py-3 text-right font-medium">{{ $settings->currency ?? 'PHP' }} {{ number_format((float) $expense->amount, 2) }}</td></tr>
            @empty
